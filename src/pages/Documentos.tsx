    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-muted mt-2">Gerencie todos os documentos e relatórios</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-text-muted text-sm font-medium">Total de Documentos</p>
              <p className="text-2xl font-bold text-text-primary mt-1">{documentos.length}</p>
            </div>
            <div className="w-12 h-12 bg-spotify-blue bg-opacity-20 rounded-lg flex items-center justify-center">
              <FileText className="w-6 h-6 text-spotify-blue" />
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-text-muted text-sm font-medium">Aprovados</p>
              <p className="text-2xl font-bold text-spotify-green mt-1">
                {documentos.filter(d => d.status === 'Aprovado').length}
              </p>
            </div>
            <div className="w-12 h-12 bg-spotify-green bg-opacity-20 rounded-lg flex items-center justify-center">
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-text-muted text-sm font-medium">Pendentes</p>
              <p className="text-2xl font-bold text-spotify-yellow mt-1">
                {documentos.filter(d => d.status === 'Pendente').length}
              </p>
            </div>
            <div className="w-12 h-12 bg-spotify-yellow bg-opacity-20 rounded-lg flex items-center justify-center">
              <FileText className="w-6 h-6 text-spotify-yellow" />
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-text-muted text-sm font-medium">Em Revisão</p>
              <p className="text-2xl font-bold text-spotify-orange mt-1">
                {documentos.filter(d => d.status === 'Em Revisão').length}
              </p>
            </div>
            <div className="w-12 h-12 bg-spotify-orange bg-opacity-20 rounded-lg flex items-center justify-center">
              <FileText className="w-6 h-6 text-spotify-orange" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          {/* Tipo Filter */}
          <div>
            <label htmlFor="tipo-filter" className="block text-sm font-medium text-text-muted mb-2">
              Tipo
            </label>
            <select id="tipo-filter" className="input-field w-full">
          ...
          <div>
            ...
            <select id="status-filter" className="input-field w-full">
              <option value="">Todos</option>
      ...
      <div className="flex flex-wrap gap-2">
        {categorias.map((categoria) => (
          <button
            key={categoria.nome}
            className="px-4 py-2 bg-background-tertiary text-text-muted rounded-lg hover:bg-spotify-green hover:text-white transition-colors duration-200 text-sm"
          >
      ...
      <div className="grid gap-4">
        {documentos.map((documento) => (
          <div key={documento.id} className="card-hover">
            <div className="flex items-start justify-between">
              <div className="flex items-start gap-4 flex-1">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-text-muted text-sm mb-3">{documento.descricao}</p>
              ...
              <div className="flex items-center gap-2 ml-4">
                ...
                <button className="btn-ghost p-2 hover:text-spotify-green">
                  <Download className="w-4 h-4" />
                </button>
                <button className="btn-ghost p-2">
                  <Edit className="w-4 h-4" />
                </button>
                <button className="btn-ghost p-2 hover:text-spotify-red">
